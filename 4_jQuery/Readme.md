...
- 클래스 선택자로 여러 요소가 선택 되었을 때, 전체 요소의 텍스트를 리턴
```js
...
- 선택된 요소의 이벤트 까지 복제 할 것인지 ? (default : false)
```js
...
1. `$('선택자').remove()` : 선택자로 선택된 요소를 제거 후 제거된 요소를 리턴한다. 단, 리턴되는 요소의 이벤트도 삭제함
2. `$('선택자').detach()` : 선택자로 선택된 요소를 제거 후 제거된 요소를 리턴한다. 단, 리턴되는 요소의 이벤트는 삭제하지 않음
3. `$('선택자').empty()` : 선택자로 선택된 요소의 하위 요소들을 삭제함
...
// or
$('선택자').each(배열, function(index, item){
    ...
});
```
...
## one 메소드
- one()메소드는 이벤트를 연결하여 한번만 실행하는 메소드
- `$('선택자').one('이벤트이름','함수')`
...
```js
$('선택자').이벤트(function(evnet){});
``` 


## hover()
- mouseenter 이벤트, mouseleave 이벤트를 동시에 연결
...
# 효과 (Effect)
- HTML 문서 내에 애니메이션 효과를 만들기 위한 메소드의 집합을 Effect라고 함

## show / hide()
- show() : 선택 요소를 보여주는 메소드
- hide() : 선택 요소를 숨기는 메소드
- toggle() : show hide 동시 적용 하는 메소드

```js
$('선택자').show/hide/toggle([time],[speed],[callback])
```

## fade()
- fadeIn() : 선택된 요소를 점점 진하게 보여주는 메소드
- fadeOut() : 선택된 요소를 점점 연하게 보여주는 메소드
- fadeTo() : 선택된 요소를 설정한 값까지 희미해지는 효과 주는 메소드
- fadeToggle() : fadeIn, fadeOut 을 동시에 적용하는 메소드
...
```js
$('선택자').slideDown/slideUp/slideToggle([시간],[구간별 속도],[callback]);
```