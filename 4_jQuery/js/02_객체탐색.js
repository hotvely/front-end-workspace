//1. Filltering 메서드
//선택된 요소들 중 첫번째 요소를 가져와서 스타일 변경 해 봄
$('h4').first().css('font-size','4rem');
$('h4:first').css('color','red');
$('h4').filter(':first').css('background-color','lightyellow');

// h4 중 마지막 요소의 글자 크기 2rem
$('h4:last').css('font-size', '2rem');
// h4 중 다섯번째 요소의 텍스트를 '테스트-5'로 변경
$('h4').eq(4).text('테스트-5');
$('h4:eq(4)').text('테스트-555');

// 클래스 선택자 : test를 선택해서 홀수 인덱스 요소만 배경색(royalblue), 글자색(white)
$('h4.test').odd().css({'background-color':'royalblue', 'color':'white'});
// $('h4.test').filter(':odd').css({});

// 클래스 선택자 : test를 선택해서 짝수 인덱스 요소 배경색(hotpink), 글자색(white)
$('.test').not(':odd').css({'background-color':'hotpink', 'color':'white'});
// $('.test').even().css({'background-color':'black', 'color':'white'});

// h4중 클래스 선택자 test가 아닌 요소를 선택해서 배경색(chocolate), 글자색(white)
$('h4').not('.test').css({'background-color':'chocolate', 'color':'white'});
$('h4:not(.test)').css({'background-color':'chocolate', 'color':'white'});
$('h4').filter(':not(.test)').css({'background-color':'chocolate', 'color':'white'});

// 선택된 요소들 중 특정 요소가 존재하는지 확인(있으면 true, 없으면 false)
console.log(` $('h4').is('.test') : ${$('h4').is('.test')}`);
...
// li 요소의 모든 상위 요소에 스타일 : 글자색(blue),
$('li').parents().css('color','blue');
// li 요소의 상위 요소 중 div만 스타일 : border(2px),style(dash),color(tomato))
$('li').parents('div').css('border','2px dashed tomato');
// span 요소부터 div요소 이전 까지 상위 요소 스타일 : 배경색(lavenderblush)
